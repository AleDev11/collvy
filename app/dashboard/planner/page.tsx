...
export default async function PlannerIndexPage() {
  ...
  // If there's exactly one project, go straight to its planner
  if (projects.length === 1) {
    redirect(`/dashboard/projects/${projects[0].id}/planner`)
  }

  // Zero or multiple projects — show picker
  const user = {
  ...
  const sidebarProjects = projects.map((p) => ({ id: p.id, name: p.name, icon: p.icon }))
          ...
          {projects.length === 0 ? (
            <div className="flex flex-1 items-center justify-center rounded-2xl border border-dashed">
              <div className="text-center">
                ...
                <p className="mt-1 text-sm text-muted-foreground">
                  Create a project to start using the planner
                </p>
                <a
            ...
            <>
              <div>
                ...
                <p className="mt-1 text-muted-foreground">Choose a project to open its planner</p>
              </div>
